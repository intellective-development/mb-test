...
class CouponTieredDecreasingBalance < Coupon
  private

  def coupon_amount(order)
    item_total = value_of_items_to_apply(order)
    amount = get_discount_amount(item_total)
    calculate_shipping_and_delivery_amount(amount, order)
  end

  # choose the correct tier
  def get_discount_amount(item_total)
    discount = price_tiers.order('minimum DESC').where('minimum <= ?', item_total).first
    discount.present? ? discount.amount : 0
  end
end
