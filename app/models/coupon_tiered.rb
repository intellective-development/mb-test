...
class CouponTiered < Coupon
  private

  def coupon_amount(order)
    item_total = value_of_items_to_apply(order)
    discount_percent = get_discount_percent(item_total)
    amount = (item_total * (discount_percent.to_f / 100.0)).round_at(2)
    calculate_shipping_and_delivery_amount(amount, order)
  end

  def get_discount_percent(item_total) # choose the correct tier
    discount_percent = price_tiers.order('minimum DESC').where('minimum <= ?', item_total)
    discount_percent.present? ? discount_percent.first.percent : 0
  end
end
