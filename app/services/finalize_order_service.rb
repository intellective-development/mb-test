class FinalizeOrderService
  ...
  # Instance methods
  #----------------------------------------
  # TODO: JM: Should we be assuming all shipments are pending?
  # TODO: JM: How do we ensure the order comes to us with only pending shipments?
  def process
    ...
    @order.shipments.each do |shipment|
      ...
      begin
        ...
        # we metric 0 to give us a % when getting the avg
    ...
    # Voiding the charges was added on ticket #4595 but it started causing some issues when the user hits twice on the finalize
    # so this method is called twice what causes an error on the second call and it was causing the original charge to be voided.
    # Examples of the error can be found on ticket #4712 so I'm adding an if to only cancel_finalize if order is finalizing.
    # I'm not sure if it will work because it is a racing condition but I think it is worth a try. If it doesn't work I think we should
    # roll back the "cancel finalize" change to stop voiding the charges since this change caused more trouble then the original issue.
    # https://github.com/minibardelivery/minibar-web/pull/3462/files
  ...
  def create_and_authorize_supplier_charge(shipment)
    if shipment.initial_charge
      charge = shipment.initial_charge
      # If we need to retry authorization for some charges e.g. heroku kills us
      # we return the ones that completed without doing anything.
      return charge if charge.authorized_or_settling?

      # TODO: JM: This is a bit lame we need to ensure the state of charges is
      # always in line with authorizations / failures / retries. Here we should
      # probably raise a sentry error as we should never get to the point where
      # a shipment is still :pending and we try to authorize again.
      return false if charge.consider_settled?
    end
